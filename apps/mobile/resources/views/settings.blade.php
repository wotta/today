...
<div class="mx-auto w-full max-w-xl px-4" style="padding-top: calc(env(safe-area-inset-top) + 1.5rem); padding-bottom: calc(env(safe-area-inset-bottom) + 2.5rem);">
    ...
    @if ($status)
        <p class="mb-5 rounded-md border border-stone-200 bg-white px-4 py-3 text-[13px] font-medium shadow-sm dark:border-stone-700 dark:bg-stone-900 {{ $status['kind'] === 'error' ? 'text-red-600 dark:text-red-400' : ($status['kind'] === 'connected' ? 'text-emerald-600 dark:text-emerald-400' : 'text-stone-500 dark:text-stone-400') }}">
            {{ $status['message'] }}
        </p>
    @endif
